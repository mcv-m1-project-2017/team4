function [evalParams_pixel, evalParams_window] = trafficSignDetection(mode,geometricFeatures, params,  dataset, method_num)
...
resultFolder =  fullfile(root, 'm1-results', 'week5', dataset,...
...
for i = 1:size(files,1)
    ...
    linearMask = linearHough(image, iMask, tri_up, tri_down, squ, i);
    linearMask = linearMask(1:size(image,1), 1:size(image,2));
    
    % Apply circular Hough to detect circular signals
    filteredMask = circularHough(image);
    
    % Add linear and circular houghs
    filteredMask = or(linearMask, filteredMask);
    ...
    if (evaluateWindow)
        if (strcmp(dataset, 'test'))
            ...
            if (size(windowCandidates,1) > 5 || size(windowCandidates,2) > 5)
                fprintf('debug here\n');
            end
        else
