function [ pixel_proposals, mask_of_positive_objects ] = linearSearch ( image, integral_image, window )
%{
Jonatan Poveda
Martí Cobos
...
%}
  ...
  for candidate = 1:size(candidates,1)
    ...
    try
      ...
      x2 = max(1, range(4));
      y2 = max(1, range(2));
      regionSum = rectangle_sum(integral_image, x1, y1, x2, y2);
      
      regions_scanned = regions_scanned + 1;
      if debug
        sprintf('Checking region %d', regions_scanned);
      end
      signalClass = checkRegion(regionSum, 0);
